...
import { buildPermissionRequest, mapPermissionOutcome, } from "../acp/permission.js";
import { SessionUpdatePump, } from "../acp/session-update-pump.js";
import { mapSessionEvent } from "./event-map.js";
import { SessionConfigService, hasProducedContent, MODEL_CONFIG_ID, THOUGHT_LEVEL_CONFIG_ID, } from "./session-config.js";
...
export class DshAgentBridge {
    ...
    constructor(ctx, options = {}) {
        ...
        this.unsubscribeApproval = ctx.on("approval/request", (req, next) => this.onApprovalRequest(req, next));
    }
    /**
    ...
    async createSession(cwd) {
        ...
        const selection = { current: initial, assembled: undefined };
    ...
    onApprovalRequest(req, next) {
        ...
        if (!state || !this.permissionChannel)
            return next();
    ...
    askPermission(state, req) {
        ...
        if (!signal)
            return pending.then(mapPermissionOutcome, () => "unavailable");
        ...
        return new Promise((resolve) => {
            ...
            pending.then((result) => finish(mapPermissionOutcome(result)), () => finish("unavailable"));
        });
    }
    // -------------------------------------------------------------------------
    ...
    async setMode(sessionId, modeId) {
        ...
        if (hasProducedContent(state.handle.agent.session)) {
            throw new RpcRequestError(ErrorCode.InvalidParams, "cannot switch mode: the session already produced content");
        }
    ...
    // Internals
    // -------------------------------------------------------------------------
    async createAgent(sessionId, cwd, selection) {
    ...
    rememberStreamedStep(state, turn, step) {
        state.streamedSteps.add(`${turn}:${step}`);
        if (state.streamedSteps.size > MAX_STREAMED_STEPS)
            this.pruneStreamedSteps(state);
    }
    /** Drop stream markers from turns older than the two most recent turns. */
    pruneStreamedSteps(state) {
        let maxTurn = 0;
        ...
        for (const key of state.streamedSteps) {
            if (Number(key.slice(0, key.indexOf(":"))) < keepFrom)
                state.streamedSteps.delete(key);
        }
    }
...
function deriveTitle(text, maxLength) {
    const flat = text.replace(/\s+/g, " ").trim();
    if (flat.length <= maxLength)
        return flat || "dsh session";
    return flat.slice(0, maxLength).trimEnd() + "…";
}
//# sourceMappingURL=dsh-agent-bridge.js.map